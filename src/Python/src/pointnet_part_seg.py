...
def get_model(point_cloud, input_label, is_training, cat_num, part_num, \
    ...
    net2 = tf_util.conv2d(net2, 128, [1,1], padding='VALID', stride=[1,1], bn_decay=bn_decay,
                        bn=True, is_training=is_training, name = "sowmya", scope='seg/conv3', weight_decay=weight_decay)
    feature_output = net2
    ...
    return net, net2, end_points, feature_output

def get_last_layers(net,net2,end_points, part_num,batch_size,num_point,weight_decay):

    net2 = tf_util.conv2d(net2, part_num, [1, 1], padding='VALID', stride=[1, 1], activation_fn=None,
                          bn=False, scope='seg/conv4_1', weight_decay=weight_decay)
...
def get_loss_finetune(l_pred, seg_pred, label, seg, weight, end_points, pairwise_distances):
    ...
    indices = tf.where(seg != -1)
    labels_squeezed = tf.squeeze(seg)
    predictions_squeezed = tf.squeeze(seg_pred)
    zeros = tf.cast(tf.zeros_like(labels_squeezed), dtype=tf.bool)
    ones = tf.cast(tf.ones_like(labels_squeezed), dtype=tf.bool)
    loc = tf.where(tf.equal(labels_squeezed, -1), zeros, ones)
